...
if [ -z "$SKIP_PROXY_TESTS" ]; then
	curl -L https://github.com/ethomson/poxyproxy/releases/download/v0.6.3/poxyproxy-0.6.3.jar >poxyproxy.jar

	echo ""
	echo "Starting HTTP proxy (Basic)..."
	java -jar poxyproxy.jar -d --address 127.0.0.1 --port 8080 --credentials foo:bar --auth-type basic --quiet &

	echo ""
	echo "Starting HTTP proxy (NTLM)..."
	java -jar poxyproxy.jar -d --address 127.0.0.1 --port 8090 --credentials foo:bar --auth-type ntlm --quiet &
fi
